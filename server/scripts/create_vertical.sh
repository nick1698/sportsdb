...
step=$((step + 1))
echo "==> ${step}) About to create vertical directory '${TARGET_DIR}'"
...
step=$((step + 1))
echo
echo "==> ${step}) Creating Django project: ${SERVICE_PKG}..."
django-admin startproject "$SERVICE_PKG" .

step=$((step + 1))
echo "==> ${step}) Creating Django app: ${API_APP}..."
python manage.py startapp "$API_APP"

step=$((step + 1))
echo "==> ${step}) Writing Dockerfile.dev..."
...
step=$((step + 1))
echo
echo "==> ${step}) Automatically update Django app..."
...
echo "====> ${step}.1) Editing '${SERVICE_PKG}/settings.py'..."
...
echo "      ✓ Done!"
echo "====> ${step}.2) Truncating '${SERVICE_PKG}/urls.py'..."
...
echo "      ✓ Done!"
echo "====> ${step}.3) Deleting '${API_APP}/tests.py'..."
if [[ -f "$TESTS_FILE" ]]; then
  rm -f "$TESTS_FILE"
fi

echo "      ✓ Done!"
echo "====> ${step}.4) Creating '${API_APP}/api.py'..."
...
step=$((step + 1))
echo
echo "==> ${step}) Auto-patch infra files"
...
echo "====> ${step}.1) Checking SQL index file clarity..."
...
echo "====> ${step}.2) Writing on SQL index file..."
...
echo "====> ${step}.3) Generating the ${DB_NAME} SQL bootstrap file..."
cat > "$DB_GEN_FILE" <<EOF
...
echo "====> ${step}.4) Updating docker compose file inserting the new ${VERTICAL} service block..."
